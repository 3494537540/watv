...
#pragma clang diagnostic pop
  ...
  // 哇TV：iOS/macOS 秒开 — 不等待「尽量少卡顿」的保守缓冲再开播。
  // 配合 Dart 侧低码率起播，双端体感接近「点开即放」。
  _player.automaticallyWaitsToMinimizeStalling = NO;
  AVPlayerItem *startupItem = _player.currentItem;
  if (startupItem != nil) {
    if (@available(iOS 10.0, macOS 10.12, *)) {
      // 前向缓冲目标约 1.5s：够出画，又不会硬等很久
      startupItem.preferredForwardBufferDuration = 1.5;
    }
  }
